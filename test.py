...
parser.add_argument('--CI', type=bool,
                    default=True, help='calculate confidence interval')
...
with torch.no_grad():
    ...
    for i_batch, (sampled_batch, path) in enumerate(testloader):
        name = path[0].split('/')[-1].replace('.h5', '')
        ...
        pred_region, pred_sdm, pred_est_boundary, pred_est_region_sdm, \
        pred_est_boundary_sdm, pred_est_cdr_region, thin_boundary = model(volume_batch)
        ...
        plt.imshow(y_pred_OC_r)
        plt.show()


        plt.imshow(y_pred_OD_r)
        plt.show()
        ...
        plt.imshow(y_pred_OC_sdm)
        plt.show()

        plt.imshow(y_pred_OD_sdm)
        plt.show()
