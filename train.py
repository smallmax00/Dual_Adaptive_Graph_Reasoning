...
parser.add_argument('--root_path', type=str,
                    default='your-own-path', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='your-own-name', help='experiment_name')
parser.add_argument('--model', type=str,
                    default='your-own-name', help='model_name')
...
parser.add_argument('--labeled_num', type=int, default=5,
                    help='labeled data')
parser.add_argument('--unlabeled_num', type=int, default=100,
                    help='labeled data')
...
def unlabeled_patients_to_slices(dataset, patiens_num):
    ref_dict = None
    if 'UKBB-CDR' in dataset:
        ref_dict = {"100": 38421, "50": 19210, "40": 9605, "30": 11526, "20": 1802, "10": 2401, "5": 1200}
...
def train(args, snapshot_path):
    ...
    unlabeled_slice = unlabeled_patients_to_slices(args.root_path, args.unlabeled_num)
    print("Total silices is: {}, labeled slices is: {}".format(
        total_slices, labeled_slice))
    labeled_idxs = list(range(0, labeled_slice))
    unlabeled_idxs = list(range(labeled_slice, unlabeled_slice))
    ...
    for epoch_num in iterator:
        for i_batch, (sampled_batch, path) in enumerate(trainloader):
            ...
            loss_dice_OC_b = bIou_loss(
                outputs_est_OC_b[:args.labeled_bs], gt_OC_b[:args.labeled_bs]) + bIou_loss(outputs_est_OC_b_sdm[:args.labeled_bs], gt_OC_b[:args.labeled_bs])

            loss_dice_OD_b = bIou_loss(
                outputs_est_OD_b[:args.labeled_bs], gt_OD_b[:args.labeled_bs]) + bIou_loss(outputs_est_OD_b_sdm[:args.labeled_bs], gt_OD_b[:args.labeled_bs])
            ...
            supervised_loss = (loss_dice_OC_r + loss_dice_OD_r + loss_mse_OC_s + loss_mse_OD_s
                               + loss_dice_OC_b / 2 + loss_dice_OD_b/2) / 6 + cdr_weight * weakly_sup_loss_cdr1 / 2
...
if __name__ == "__main__":
    ...
    snapshot_path = "your-own-path".format(
        args.exp, args.labeled_num,args.unlabeled_num, args.model, args.batch_size, args.dropout)
