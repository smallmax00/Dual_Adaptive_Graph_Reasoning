...
class MSRF(nn.Module):  # multi scale receptive field
...
class ODOC_cdr_graph(nn.Module):
    ...
    def __init__(self, channel=64, k1=5000, k2=70, dropout=0.1):
        ...
        # self.graph_node_b = graph_node_b(channel=channel, out_channel=2)
        self.graph_node_s = graph_node_s(channel=channel, out_channel=2)

        self.GCN_r_b = Res_EAGCN(num_in=2, plane_mid=1, mids=64)
        self.GCN_s_b = Res_EAGCN(num_in=2, plane_mid=1, mids=64)
        ...
        # min_k: kernal_size control the width of boundary e.g. 3->1, 5->2, 7->3, 9->4
        self.s_region = SDM_region_detector(k1, k2)
        # self.s_region = SDM_region_detector()
    ...
    def forward(self, x):
        ...
        x1 = self.resnet.layer1(x)  # [bs, 256, 32, 32]

        x2 = self.resnet.layer2(x1)  # [bs, 512, 32, 32]

        x3 = self.resnet.layer3(x2)  # [bs, 1024, 16, 16]

        x4 = self.resnet.layer4(x3)  # [bs, 2048, 8, 8]


        x2_o = self.rfb3_1(x2)  # [bs, 128, 32, 32]

        x3_o = self.rfb4_1(x3)  # [bs, 128, 16, 16]

        x4_o = self.rfb5_1(x4)  # [bs, 128, 8, 8]

        x_all_r = self.graph_node_r(x4_o, x3_o, x2_o)  # [bs, 2, 32, 32]
        x_all_r = F.interpolate(x_all_r, scale_factor=2, mode='bilinear', align_corners=True)  # [bs, 2, 64, 64]
        x_all_s = self.graph_node_s(x4_o, x3_o, x2_o)  # [bs, 2, 32, 32]
        x_all_s = F.interpolate(x_all_s, scale_factor=2, mode='bilinear', align_corners=True)  # [bs, 2, 64, 64]

        if self.training:
            x_all_r = self.dropout(x_all_r)
        r_gcn = self.GCN_r_b(x_all_r, x_all_s)
        r_gcn = x_all_r + r_gcn

        if self.training:
            x_all_s = self.dropout(x_all_s)
        s_gcn = self.GCN_s_b(x_all_s, x_all_r)
        s_gcn = s_gcn + x_all_s
        ...
        # outputs_sdm = torch.sigmoid(s_gcn)  # [0-1, 0]
        outputs_sdm = torch.tanh_(s_gcn)  # [0-1, -1]
        ...
        return outputs_region, outputs_sdm, outputs_est_boundary, \
               outputs_est_region_sdm, outputs_est_boundary_sdm, outputs_est_cdr_region
